...
import {
  ...
  ShoppingBag,
  Truck,
} from "lucide-react"; // 2. Added ShoppingBag & Truck
...
export default async function AdminDashboard() {
  ...
  const totalOrdersCount = await db
    ...
    .from(orders);
  const activePaidOrders = await db
    .select({ count: sql<number>`count(*)` })
    .from(orders)
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-gray-200 pb-6 gap-4">
          <div>
            ...
            <p className="text-[#8c363e] text-xs uppercase tracking-widest mt-2 font-bold">
              Everafter x Rinku Video Lab
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                {activePaidOrders[0]?.count || 0} orders waiting for Delhivery
                allocation • Total logged orders:{" "}
                {totalOrdersCount[0]?.count || 0}
              </p>
            </div>
          </div>
          <Link
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-serif text-2xl text-gray-900 flex items-center gap-2">
              <Star size={20} className="text-[#d99898] fill-[#d99898]" />{" "}
              Homepage Showcase
            </h2>
          ...
          {featuredReels.length === 0 ? (
            <div className="p-8 border border-dashed border-gray-300 rounded-lg bg-gray-50 text-center">
              <p className="text-gray-500 text-sm">
                No reels are currently featured on the homepage. Toggle one on
                below!
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {featuredReels.map((reel) => {
                ...
                return (
                  <div
                    key={reel.id}
                    className="relative rounded-xl overflow-hidden border-2 border-[#d99898] shadow-sm group"
                  >
                    <div className="aspect-[9/16] bg-black relative">
                      ...
                      <div className="absolute bottom-3 left-3 right-3">
                        <p className="text-white font-bold text-[10px] uppercase truncate">
                          {linkedGown?.name || "No Gown Linked"}
                        </p>
                      </div>
                    </div>
                    {/* Remove from Homepage Quick Action */}
                    <form
                      action={toggleFeaturedReel.bind(null, reel.id, true)}
                      className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      <button
                        type="submit"
                        className="bg-white text-red-600 p-1.5 rounded-full shadow-lg hover:bg-red-50 text-[10px] font-bold"
                      >
                        Remove
                      </button>
                    </form>
              ...
              <tbody className="divide-y divide-gray-50">
                {allReels.map((reel) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <form
                        action={toggleFeaturedReel.bind(
                          null,
                          reel.id,
            ...
            {allReels.length === 0 && (
              <div className="p-16 text-center">
                <p className="font-serif text-lg text-gray-400 italic">
                  No cinematic reels uploaded yet.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="border border-gray-100 rounded-sm bg-white overflow-hidden shadow-sm">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-50">
                {allGowns.map((item) => {
                  const firstImage =
                    item.imageUrls?.split(",")[0] || "/placeholder.jpg";
